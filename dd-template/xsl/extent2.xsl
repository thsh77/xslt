...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template name="extent2">
        <extent>
            <dimensions unit="cm">
                <height>
                    <xsl:value-of select="manuscriptHeight" />
                    <xsl:if test="manuscriptPlica!=0">
                        <xsl:text> (plica: </xsl:text>
                        <xsl:value-of select="manuscriptPlica" />
                        <xsl:text>)</xsl:text>
                    </xsl:if>
                </height>
                <width>
                    <xsl:value-of select="manuscriptWidth" />
                </width>
            </dimensions>
        </extent>
    </xsl:template>
