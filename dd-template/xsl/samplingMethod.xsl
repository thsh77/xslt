...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="samplingMethod">
        <ab>
            <xsl:apply-templates select="textCompleteness"/>
            <xsl:apply-templates select="sourceSigil"/>
            <xsl:apply-templates select="samplingNote"/>
        </ab>
    </xsl:template>
    <xsl:template match="textCompleteness">
        <xsl:choose>
            <xsl:when test="contains(., 'version')">
                <xsl:text>Tekst efter </xsl:text>
            </xsl:when>
            <xsl:when test="contains(., 'excerpt')">
                <xsl:text>Udtog efter </xsl:text>
            </xsl:when>
            <xsl:otherwise>
                <xsl:text/>
            </xsl:otherwise>
    ...
    <xsl:template match="sourceSigil">
        <ref>
            <xsl:value-of select="text()"/>
        </ref>
        <xsl:text>. </xsl:text>
    </xsl:template>
    <xsl:template match="samplingNote">
        <xsl:if test="not(contains(., 'empty'))">
            <xsl:apply-templates/>
            <xsl:text>.</xsl:text>
        </xsl:if>
    </xsl:template>
